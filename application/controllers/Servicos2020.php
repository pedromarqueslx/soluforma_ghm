<?php
class Servicos2020 extends CI_Controller {
    ...
    $data['servicos'] = $this->servicos2020_model->get_servicos_by_id($id);
    //$data['servicos_item'] = $this->servicos2020_model->get_servicos_by_id($id);
    ...
    $this->load->view('servicos2020/pdf', $data);
    }
    ...
    $data['funcionarios'] = $this->funcionarios_model->get_funcionarios_by_id($id);
    $data['conteudos'] = $this->conteudos_model->get_conteudos_by_id($id);
    $this->load->view('servicos2020/pdf2', $data);
    }
    ...
    $config["total_rows"] = $this->servicos2020_model->record_count();
    $config["per_page"] = '';
    ...
    $data["results"] = $this->servicos2020_model->fetch_servicos($config["per_page"], $page);
    $data["links"] = $this->pagination->create_links();   
    $config["total_rows"] = $this->servicos2020_model->record_count();
    if (empty($config["total_rows"])) {
    $this->load->view('templates/header', $data);
    $this->load->view('servicos2020/view', $data);
    $this->load->view('templates/footer');
    }else{              
    $this->load->view('templates/header', $data);
    $this->load->view('servicos2020/index', $data);
    $this->load->view('templates/footer');
    }
    }else{
    ...
    $result = $this->servicos2020_model->get_autocomplete($search_data);
    if (!empty($result)) {
    ...
    // load model
    $search_data = $this->input->post('search_data');
    $result = $this->servicos2020_model->get_autocomplete_create($search_data);
    if (!empty($result)) {
    echo "<table class='ink-table hover alternating'>";
    foreach ($result as $row):
    echo "<tr><td>";
    ...
    $data['servicos_item'] = $this->servicos2020_model->get_servicos($slug);
    ...
    $data['servicos'] = $this->servicos2020_model->get_servicos();
    ...
    $this->load->view('templates/header', $data);
    $this->load->view('servicos2020/create');
    $this->load->view('templates/footer');
    } else {
    $this->servicos2020_model->set_servicos($data);
    redirect( base_url() . 'index.php/servicos2020/index/');
    }}else{
    // if user is NOT loggedIn redirect to homepage
    redirect( base_url() );
    }
    ...
    public function getFuncionariosbyempresa() {
        ...
        // get data
        $data = $this->servicos2020_model->getEmpresas($postData);
    ...
    public function getConteudosbyareaformacao() {
        ...
        $postData = $this->input->post();
        // get data
        $data = $this->servicos2020_model->getConteudos($postData);
        echo json_encode($data);
    }
    ...
    public function getConteudosbyIDareaformacao() {
        ...
        $postData = $this->input->post();
        // get data
        $data = $this->servicos2020_model->getConteudosbyID($postData);
        echo json_encode($data);
    }
    ...
    if ($this->form_validation->run() === FALSE) {
    $this->load->view('templates/header', $data);
    $this->load->view('servicos2020/edit', $data);
    $this->load->view('templates/footer');
    }else{
        $this->servicos2020_model->set_servicos_id($id);
        redirect( base_url() . 'index.php/servicos2020/index/');
    }}else{
    // if user is NOT loggedIn redirect to homepage
    redirect( base_url() );
    ...
    $data['equipamentos_item'] = $this->servicos2020_model->get_servicos_by_title($id);
    $data['servicos'] = $this->servicos2020_model->get_servicos_by_title();
    $data['show_table'] = $this->view_table();
    $this->load->view('templates/header', $data);
    $this->load->view('servicos2020/total', $data);
    $this->load->view('templates/footer');
    } else {
    // if user is NOT loggedIn redirect to homepage
    redirect( base_url() );
    ...
    $servicos_item = $this->servicos2020_model->get_servicos_by_id($id);
    $this->servicos2020_model->delete_servicos($id);
    redirect( base_url() . 'index.php/servicos2020');
    }

    // function to get all artigos records :: COMMENT LATER
    public function view_table() {
    $result = $this->servicos2020_model->show_all_data();
    if ($result != false) {
    return $result;
    }else{
    ...
    public function select_id_and_date_range() {
        ...
        if ($date1 == "" || $date2 == "" || $id == "") {
            ...
            $result = $this->servicos2020_model->select_id_and_date_range($data);
            if ($result != false) {
