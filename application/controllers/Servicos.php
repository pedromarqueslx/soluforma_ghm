...
class Servicos extends CI_Controller {
    ...
    $data['servicos'] = $this->servicos_model->get_servicos_by_id($id);
    //$data['servicos_item'] = $this->servicos_model->get_servicos_by_id($id);
    ...
    $this->load->view('servicos/pdf', $data);
    }
    ...
    $data['funcionarios'] = $this->funcionarios_model->get_funcionarios_by_id($id);
    $data['conteudos'] = $this->conteudos_model->get_conteudos_by_id($id);
    $this->load->view('servicos/pdf2', $data);
    }
    ...
    $config["total_rows"] = $this->servicos_model->record_count();
    $config["per_page"] = '';
    ...
    $data["results"] = $this->servicos_model->fetch_servicos($config["per_page"], $page);
    ...
    $result = $this->servicos_model->get_autocomplete($search_data);
    if (!empty($result)) {
    ...
    // load model
    $search_data = $this->input->post('search_data');
    $result = $this->servicos_model->get_autocomplete_create($search_data);
    if (!empty($result)) {
    echo "<table class='ink-table hover alternating'>";
    foreach ($result as $row):
    echo "<tr><td>";
    ...
    $data['servicos_item'] = $this->servicos_model->get_servicos($slug);
    ...
    $this->load->view('templates/header', $data);
    $this->load->view('servicos/create');
    $this->load->view('templates/footer');
    }else{
    //$this->contactos_model->set_contactos();
    $this->servicos_model->set_servicos($data);
    //$insert_id = $this->db->insert_id('servicos');
    redirect( base_url() . 'index.php/servicos/index/');
    }}else{
    // if user is NOT loggedIn redirect to homepage
    redirect( base_url() );
    }
    ...
    public function getFuncionariosbyempresa() {
        ...
        // get data
        $data = $this->servicos_model->getEmpresas($postData);
    ...
    public function getConteudosbyareaformacao() {
        ...
        $postData = $this->input->post();
        // get data
        $data = $this->servicos_model->getConteudos($postData);
        echo json_encode($data);
    }
    ...
    public function getConteudosbyIDareaformacao() {
        ...
        $postData = $this->input->post();
        // get data
        $data = $this->servicos_model->getConteudosbyID($postData);
        echo json_encode($data);
    }
    ...
    $funcionario_id = '679';
    //echo $funcionario_id;
    echo $this->input->post('formandos_servicos'); 
    ...
    if ($this->form_validation->run() === FALSE) {
    $this->load->view('templates/header', $data);
    $this->load->view('servicos/edit', $data);
    $this->load->view('templates/footer');
    }else{
        $this->servicos_model->set_servicos_id($id);
        redirect( base_url() . 'index.php/servicos/index/');
    }}else{
    // if user is NOT loggedIn redirect to homepage
    redirect( base_url() );
    ...
    $servicos_item = $this->servicos_model->get_servicos_by_id($id);
    ...
    $result = $this->servicos_model->show_all_data();
    if ($result != false) {
    return $result;
    }else{
    ...
    public function select_id_and_date_range() {
        ...
        if ($date1 == "" || $date2 == "" || $id == "") {
            ...
            $result = $this->servicos_model->select_id_and_date_range($data);
            if ($result != false) {
