<?php
class Formacoes2021 extends CI_Controller {
    ...
    $this->load->model('formacoes2021_model');
    ...
    $data['formacoes'] = $this->formacoes2021_model->get_formacoes_by_id($id);
    ...
    $this->load->view('formacoes2021/pdf', $data);
    }
    ...
    $data['formacoes'] = $this->formacoes2021_model->get_formacoes_group();
    if ($this->form_validation->run() === FALSE) {
    $this->load->view('templates/header', $data);
    $this->load->view('formacoes2021/index');
    ...
    $result = $this->formacoes2021_model->get_autocomplete($search_data);
    if ( !empty ($result) ) {
    ...
    // load model
    $search_data = $this->input->post('search_data');
    $result = $this->formacoes2021_model->get_autocomplete_create($search_data);
    if ( !empty ($result) ) {
    echo "<table class='ink-table hover alternating'>";
    foreach ($result as $row):
    echo "<tr><td>";
    ...
    $data['formacoes_item'] = $this->formacoes2021_model->get_formacoes($slug);
    if (empty($data['formacoes_item']))
    {show_404();}
    $data['title'] = $data['formacoes_item']['title'];
    $this->load->view('templates/header', $data);
    $this->load->view('formacoes2021/view', $data);
    $this->load->view('templates/footer');
    }
    ...
    $data['formacoes'] = $this->formacoes2021_model->get_formacoes();
    ...
    if ($this->form_validation->run() === FALSE) {
    $this->load->view('templates/header', $data);
    $this->load->view('formacoes2021/create');
    $this->load->view('templates/footer');
    } else {
    $this->formacoes2021_model->set_formacoes();
    redirect( base_url() . 'index.php/formacoes2021/index/');
    }
    ...
    public function getFuncionariosbyempresa() {
        ...
        // get data
        $data = $this->formacoes2021_model->getEmpresas($postData);
    ...
    public function getConteudosbyareaformacao() {
        ...
        $postData = $this->input->post();
        // get data
        $data = $this->formacoes2021_model->getConteudos($postData);
        echo json_encode($data);
    }
    ...
    $data['formacoes_item'] = $this->formacoes2021_model->get_formacoes_by_slug($id);
    $this->load->view('templates/header', $data);
    $this->load->view('formacoes2021/edit', $data);
    $this->load->view('templates/footer');
    }else{
    // if user is NOT loggedIn redirect to homepage
    redirect( base_url() );
    ...
    if (empty($id)) {show_404();}
    $data['title'] = 'Editar Registo';
    $data['formacoes_item'] = $this->formacoes2021_model->get_funcionarios_sem_formacao_by_slug($id);
    $this->load->view('templates/header', $data);
    $this->load->view('formacoes2021/funcionarios', $data);
    $this->load->view('templates/footer');
    }else{
    // if user is NOT loggedIn redirect to homepage
    redirect( base_url() );
    ...
    $data['equipamentos_item'] = $this->formacoes2021_model->get_formacoes_by_title($id);
    $data['formacoes'] = $this->formacoes2021_model->get_formacoes_by_title();
    $data['show_table'] = $this->view_table();
    $this->load->view('templates/header', $data);
    $this->load->view('formacoes2021/total', $data);
    $this->load->view('templates/footer');
    } else {
    // if user is NOT loggedIn redirect to homepage
    redirect( base_url() );
    ...
    $formacoes_item = $this->formacoes2021_model->get_formacoes_by_id($id);
    $this->formacoes2021_model->delete_formacoes($id);
    redirect( base_url() . 'index.php/formacoes2021');
    }

    // function to get all artigos records :: COMMENT LATER
    public function view_table() {
    $result = $this->formacoes2021_model->show_all_data();
    if ($result != false) {
    return $result;
    }else{
    ...
    public function select_id_and_date_range() {
        ...
        if ($date1 == "" || $date2 == "" || $id == "") {
            ...
            $result = $this->formacoes2021_model->select_id_and_date_range($data);
            if ($result != false) {
