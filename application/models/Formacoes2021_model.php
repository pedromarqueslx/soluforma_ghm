...
class Formacoes2021_model extends CI_Model {
...
public function select_formacoes($id = 0) {
    if ($id === 0) {
    $query = $this->db->get('servicos2021');
    return $query->result_array();
    }
    $query = $this->db->get_where('servicos2021', array('id' => $id));
...
public function get_autocomplete($search_data) {
    ...
    return $this->db->where('visivel_servicos', '1')->where('categoria_servicos', '1')->get('servicos2021', 10)->result();
}
...
public function get_formacoes($slug = FALSE) {
    ...
    $query = $this->db->get_where('servicos2021', array('slug' => $slug));
    return $query->row_array();
}
...
public function get_funcionarios_sem_formacao_by_slug($id = 0) {
    ...
    $funcionarios_com_formacao = $this->db->get('servicos2021')->result_array();
    ...
    $this->db->from('funcionarios', 'servicos2021.empresa_id_servicos');
...
public function get_formacoes_by_slug($id = 0) {
    ...
    $this->db->select('*');
    $this->db->select('servicos2021.nome_funcionario_servicos AS nome_funcionario_servicos, servicos2021.nome_servicos AS nome_servicos, servicos2021.data_servicos AS data_servicos, servicos2021.horas_servicos AS horas_servicos');
    $this->db->from('servicos2021');
...
public function get_formacoes_group($slug = FALSE) {
    if ($slug === FALSE) {
        ...
        $this->db->from('servicos2021 AS certificados');
...
public function get_formacoes_by_title($id = 0) {
    ...
    $query = $this->db->get_where('servicos2021', array('title' => $id));
    return $query->row_array();
}
...
public function set_formacoes_id($id = 0) {
    ...
    if ($id == 0) {
    return $this->db->insert('servicos2021', $data);
    }else{
        $this->db->where('id', $id);
        return $this->db->update('servicos2021', $data);
    }
}

public function delete_formacoes($id) {
    $this->db->where('id', $id);
    return $this->db->delete('servicos2021');
}
...
public function record_count() {
    return $this->db->count_all("servicos2021");
}
...
public function show_all_data() {
    $this->db->select('*');
    $query = $this->db->where('visivel_servicos', '1')->where('categoria_servicos', '1')->get('servicos2021');
