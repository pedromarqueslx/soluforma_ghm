<?php
class inspeccao_model extends CI_Model {
    ...
    public function get_autocomplete($search_data)
    {
        ...
        return $this->db->where('visivel_artigo', '1')->where('categoria_artigo', '10')->get('artigos', 10)->result();
    }

	public function get_inspeccao($slug = FALSE)
	{
        if ($slug === FALSE)
        {
    ...
    public function get_inspeccao_by_id($id = 0)
    {
        if ($id === 0)
        {
            $query = $this->db->get('artigos');
    ...
    public function get_inspeccao_by_title($title = 0)
    {
        if ($title === 0)
        {
            $title = $this->uri->segment(3);
            $query = $this->db->where('visivel_artigo', '1')->where('categoria_artigo', '10')->where('title', $title)->get('artigos');
            return $query->result_array();
        }
    ...
    public function set_inspeccao($id = 0)
    {
    ...
    public function delete_inspeccao($id)
    {
        $this->db->where('id', $id);
    ...
    public function record_count() {
        $query = $this->db->where('visivel_artigo', '1')->where('categoria_artigo', '10')->get('artigos');
        return $query->num_rows();
    }

    public function fetch_inspeccao($limit, $start) {
    ...
    public function select_id_and_date_range($data) {
        ...
        $this->db->where($condition)->where('visivel_artigo', '1')->where('categoria_artigo', '10');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result();
        } else {
