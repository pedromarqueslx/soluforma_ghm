<?php
class prestacao_model extends CI_Model {
    ...
    public function get_autocomplete($search_data)
    {
        ...
        return $this->db->get('artigos', 10)->result();
    }    

	public function get_prestacao($slug = FALSE)
	{
        if ($slug === FALSE)
        {
    ...
    public function get_prestacao_by_id($id = 0)
    {
        if ($id === 0)
        {
            $query = $this->db->get('artigos');
    ...
    public function get_prestacao_by_title($title = 0)
    {
        if ($title === 0)
        {
            $title = $this->uri->segment(3);
            $query = $this->db->where('visivel_artigo', '1')->where('categoria_artigo', '12')->where('title', $title)->get('artigos');
            return $query->result_array();
        }
    ...
    public function set_prestacao($id = 0)
    {
    ...
    public function delete_prestacao($id)
    {
        $this->db->where('id', $id);
    ...
    public function record_count() {
        $query = $this->db->where('visivel_artigo', '1')->where('categoria_artigo', '12')->get('artigos');
        return $query->num_rows();
    }

    public function fetch_prestacao($limit, $start) {
